...
// Send a background ping to wake the server up immediately when the user loads the page
fetch(`${API_BASE_URL}/`).catch(() => {});
// Ping every 10 minutes to keep it awake while the user is working
setInterval(() => {
    fetch(`${API_BASE_URL}/`).catch(() => {});
}, 10 * 60 * 1000);
...
async function runClustering() {
    ...
    try {
        ...
        if (err.message === "Failed to fetch") {
            errEl.innerText = "❌ Server is waking up from sleep. Please wait 45 seconds and click Start again.";
        } else {
